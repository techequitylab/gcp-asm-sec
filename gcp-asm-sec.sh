...
mkdir -p `pwd`/gcp-asm-sec > /dev/null 2>&1
export PROJDIR=`pwd`/gcp-asm-sec
export SCRIPTNAME=gcp-asm-sec.sh
...
EOF
    ...
    echo
    echo "$ kubectl annotate --overwrite namespace default mesh.cloud.google.com/proxy='{\"managed\":\"true\"}' # to enable Google to manage data plane"
